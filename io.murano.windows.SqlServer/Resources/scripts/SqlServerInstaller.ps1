...
function New-AlwaysOnAvailabilityGroup {
    ...
    for ($i = 0; $i -lt $DatabaseNames.Length; $i++) {
        ...
        New-SQLDatabase $DataBaseName
        $BackupDb = "
            BACKUP DATABASE $(ConvertTo-SQLName $DataBaseName) TO DISK = N$(ConvertTo-SQLString "$WorkDir\db$i.bak") WITH NOFORMAT, INIT, NAME = N'Full Database Backup', SKIP, NOREWIND, NOUNLOAD, STATS = 10
            GO"
        [void](Invoke-SQLText -SQL $BackupDb)
        $BackupLog = "
            BACKUP LOG $(ConvertTo-SQLName $DataBaseName) TO DISK = N$(ConvertTo-SQLString "$WorkDir\db${i}.log.bak") WITH NOFORMAT, INIT,  NAME = N'Transaction Log  Backup', SKIP, NOREWIND, NOUNLOAD, STATS = 10
            GO"
        [void](Invoke-SQLText -SQL $BackupLog)
    }
    $ReplicaDefinitionsArray = @()
    ...
    $SQL = "
        CREATE AVAILABILITY GROUP $(ConvertTo-SQLName $Name) $PrefsLine
